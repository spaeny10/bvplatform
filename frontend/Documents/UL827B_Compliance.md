...
| # | Control | Status | Evidence | Notes |
|---|---|---|---|---|
| A.13 | Dual-operator verification on dispositioned events | ✅ | `security_events.disposed_by_user_id`, `verified_by_user_id`, `verified_by_callsign`, `verified_at` columns; `VerifySecurityEvent` in [`internal/database/platform_db.go`](../../internal/database/platform_db.go); handler at [`internal/api/platform.go`](../../internal/api/platform.go) `HandleVerifySecurityEvent`; route `POST /api/v1/events/{id}/verify`; supervisor UI at [`VerificationQueueCard`](../../frontend/src/components/reports/VerificationQueueCard.tsx) on `/reports` Verification tab | Restricted to `soc_supervisor` and `admin` roles. The verify update is one atomic conditional UPDATE that rejects self-verification (`disposed_by_user_id <> verifier`) and re-verification (`verified_at IS NULL`). 409 distinguishes "already verified" from "self-verify attempt." Verification queue card auto-refreshes every 30s, hides the verify button when the supervisor is the disposing operator, and shows the AVS score next to each pending event. |
...
| D.2 | Public share token (full lifecycle) | ✅ | [`internal/api/evidence_share.go`](../../internal/api/evidence_share.go) `HandleCreateEvidenceShare`, `HandleRevokeEvidenceShare`, `HandleListEvidenceShares`; DB methods in [`internal/database/soc_ids.go`](../../internal/database/soc_ids.go); supervisor UI at [`EvidenceSharesCard`](../../frontend/src/components/reports/EvidenceSharesCard.tsx) on `/reports` Evidence shares tab | Tokens are 256-bit URL-safe random. Creation restricted to soc_supervisor/admin (audited as `create_evidence_share`). Default TTL 7 days, hard ceiling 90 days — UL 827B reviewers reject "never expires" share links so the cap is enforced server-side. List endpoint denormalizes `open_count` from `evidence_share_opens` so the manage-shares UI shows access volume per token. Revoked or expired tokens return 404 publicly (no information leak). Supervisor card lets a supervisor enter an incident ID, see all tokens (active/revoked/expired) with their open counts, and revoke a live token instantly. |
...
| E.2 | Operator response-time reporting | ✅ | Backend at [`internal/api/reports.go`](../../internal/api/reports.go) `HandleSLAReport`; supervisor UI at [`frontend/src/app/reports/page.tsx`](../../frontend/src/app/reports/page.tsx) → Performance tab consuming [`SLAReportCard`](../../frontend/src/components/reports/SLAReportCard.tsx) | Query parameters: `from`, `to` (RFC3339, default 30 days), `group=operator|day`, `format=json|csv`. Returns total/acked/within-SLA/over-SLA counts plus avg, p50, p95 ack-time-in-seconds per bucket. Surfaced to supervisors and admins on `/reports` with date-range presets (7d/30d/90d) and a one-click CSV download for audit deliverables. |
