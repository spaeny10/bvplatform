...
// All 9 Milesight Advanced tab bodies (P1-B-11 session 4). One file
// instead of nine because the tabs share the same `usePanel`+`useSaver`
// load/save pattern and look-and-feel — splitting them further would
// fragment without making each one easier to find. The parent
// MilesightAdvanced.tsx is now a thin tab router that imports from here.

import { useState } from 'react';
import {
    milesightReboot, milesightPTZGoto,
...
import {
    usePanel, useSaver, Row, SaveBar,
    inputStyle, btnDanger, btnSecondary,
    StreamEditor,
} from './shared';

// ── Streams (resolution / framerate / bitrate / codec per stream) ──

export function StreamsTab({ cameraId }: { cameraId: string }) {
...
export function OSDTab({ cameraId }: { cameraId: string }) {
...
export function ImageTab({ cameraId }: { cameraId: string }) {
...
export function AudioTab({ cameraId }: { cameraId: string }) {
...
export function NetworkTab({ cameraId }: { cameraId: string }) {
...
export function PrivacyTab({ cameraId }: { cameraId: string }) {
...
export function SystemTab({ cameraId }: { cameraId: string }) {
...
export function PTZTab({ cameraId }: { cameraId: string }) {
...
export function AlarmTab({ cameraId }: { cameraId: string }) {
